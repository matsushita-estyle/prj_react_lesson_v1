...
import TextBookSection from '@/components/organisms/TextBookSection'
...
const LessonTextBookModal: React.FC<LessonTextBookModalProps> = ({ isOpen, onClose, lesson }) => {
  ...
  // 新形式のtextBookSectionsがある場合はそれを使用、なければ従来のtextBookを使用
  const hasTextBookSections = lesson.textBookSections && lesson.textBookSections.length > 0
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(80vh-120px)]">
          {hasTextBookSections ? (
            ...
            <div className="space-y-6">
              {lesson.textBookSections!
                .sort((a, b) => (a.order || 0) - (b.order || 0))
                .map((section) => (
                  <TextBookSection key={section.id} section={section} />
                ))}
            </div>
          ) : (
            /* 従来形式: 単一のMarkdownコンテンツ */
